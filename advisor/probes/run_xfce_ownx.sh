...
echo STAGE_WESTON
weston --backend=drm-backend.so --socket=wayland-0 --use-pixman --shell=kiosk-shell.so --logger-scopes=log > /tmp/weston.out 2>&1 &
...
# Start our OWN rootful Xwayland on :1 rather than waiting for weston's.
#
# This is the fix for "Another Window Manager (Weston WM) is already running on screen :0.0".
# The WM selection is NOT held by weston's shell -- proven by testing kiosk-shell, which changed
# nothing -- but by weston's Xwayland integration: the strings "Weston WM" and "WM_S0" live in
# libweston-14/xwayland.so. Weston auto-launches its own Xwayland WITH a built-in X11 window
# manager that owns WM_S0 on :0. Previously this script merely WAITED for an X socket to appear,
# so XFCE joined weston's display and xfwm4 could never own the WM role there -- and --replace
# cannot take it either, because that WM does not honour ICCCM replacement.
#
# A separate rootful Xwayland has no window manager of its own, so xfwm4 owns WM_S0 outright and
# is free to set _NET_WORKAREA / _NET_NUMBER_OF_DESKTOPS, which is what xfdesktop needs.
# -fullscreen gives XFCE a real full-screen root window; -noreset keeps the server alive across
# client exits; -glamor off matches the software (pixman) compositor.
...
echo STAGE_XFWM4
# No WM-replace flag, deliberately. weston runs kiosk-shell.so above rather than
# desktop-shell.so: desktop-shell is weston's OWN full desktop (background, panel and window
# management bundled together) and it holds the WM selection on :0.0, which is why xfwm4
# previously exited with "Another Window Manager (Weston WM) is already running on screen :0.0 ...
# Could not find a screen to manage". Forcing a replace would work only by winning a race against
# a competing WM implementation. kiosk-shell does minimal surface compositing with no desktop UI
# or WM behaviour of its own, so xfwm4 is simply the only window manager present and
# _NET_WORKAREA / _NET_NUMBER_OF_DESKTOPS get a single authoritative source. It also stops weston
# compositing a background and panel that XFCE is about to paint over.
