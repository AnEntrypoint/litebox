...
# xfwm4 is NOT launched here. weston's XWM already owns the X root window
# ("created wm, root 98"), and X11 permits exactly ONE client to hold
# SubstructureRedirect on it. Since weston's XWM is what maps X surfaces into
# the scene graph, a second window manager contesting the root may be exactly
# what stops anything being drawn.
echo STAGE_XFWM4_SKIPPED
...
# The previous run ended 14s after the panel started, and weston only flips on
# damage -- so no frame was ever captured while XFCE was actually up. Wait long
# enough for the desktop to finish drawing, and poke the display periodically so
# an idle-but-drawn desktop still produces a flip we can capture.
echo SETTLE_START
i=0
while [ "$i" -lt 30 ]; do
    i=$((i + 1))
    # xfdesktop repaints on a background-property change; this is a cheap nudge
    # that generates damage without needing any extra tooling in the layer.
    [ $((i % 20)) -eq 0 ] && xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitor0/workspace0/color-style -n -t int -s 0 2>/dev/null
    sleep 0.5
done
echo SETTLE_DONE

# Did the components WRITE a configuration for themselves? The layer ships no
# xfce4-panel.xml or xfce4-desktop.xml, so either the panel's own migrate step
# created one (in which case it had a layout and chose to draw nothing -- a real
# bug) or it did not (in which case there is simply nothing to draw, and the gap
# is layer content rather than code).
echo "=== BEGIN weston.out ==="
# weston's XWM logs every X window it manages. If XFCE's windows are never
# mapped into the scene graph, this is where it shows -- and it is the one
# source that can say so directly rather than by inference.
tail -50 /tmp/weston.out 2>/dev/null || echo "(none)"
echo "=== END weston.out ==="
echo "=== BEGIN guest xfconf state ==="
ls -la "$HOME/.config/xfce4/xfconf/xfce-perchannel-xml/" 2>&1 || echo "(no per-channel dir)"
echo "--- panel channel ---"
xfconf-query -c xfce4-panel -l 2>&1 | head -20 || echo "(panel channel unreadable)"
echo "--- desktop channel ---"
xfconf-query -c xfce4-desktop -l 2>&1 | head -20 || echo "(desktop channel unreadable)"
echo "=== END guest xfconf state ==="
