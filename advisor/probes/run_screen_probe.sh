...
# SCREEN CAPABILITY PROBE. A GTK app that cannot find a usable screen or visual
# initialises, sits idle, and never draws -- with no error. That matches every
# observation, so establish directly whether the X screen is usable rather than
# inferring it. xfce4-display-settings queries RandR/screen geometry and prints
# what it finds; xfce4-appfinder is a normal GTK client for comparison.
echo PROBE_START
# --help does NOT open a display, so it proves nothing about the X screen.
# Run it FOR REAL: xfce4-display-settings queries RandR and will say plainly if
# it cannot get a screen, and --version exits after GTK init on a real display.
DISPLAY=$DISP GDK_BACKEND=x11 xfce4-display-settings --version > /tmp/p_help.out 2>&1
echo "PROBE_VERSION_RC=$?"
DISPLAY=$DISP GDK_BACKEND=x11 timeout 25 xfce4-display-settings > /tmp/p_real.out 2>&1
echo "PROBE_REAL_RC=$?"
# GDK's own X error reporting is always compiled in, unlike GTK_DEBUG.
DISPLAY=$DISP GDK_BACKEND=x11 GDK_SYNCHRONIZE=1 timeout 25 xfce4-appfinder > /tmp/p_sync.out 2>&1
echo "PROBE_SYNC_RC=$?"
DISPLAY=$DISP GDK_BACKEND=x11 GTK_DEBUG=all xfce4-appfinder > /tmp/p_gtk.out 2>&1 &
i=0; while [ "$i" -lt 60 ]; do i=$((i+1)); sleep 0.5; done
echo PROBE_WAITED
echo "=== BEGIN p_help.out ==="
head -25 /tmp/p_help.out 2>/dev/null || echo "(none)"
echo "=== END p_help.out ==="
echo "=== BEGIN p_real.out ==="
head -20 /tmp/p_real.out 2>/dev/null || echo "(none)"
echo "=== END p_real.out ==="
echo "=== BEGIN p_sync.out ==="
head -20 /tmp/p_sync.out 2>/dev/null || echo "(none)"
echo "=== END p_sync.out ==="
echo "=== BEGIN p_gtk.out ==="
head -60 /tmp/p_gtk.out 2>/dev/null || echo "(none)"
echo "=== END p_gtk.out ==="
echo "=== BEGIN weston.out ==="
tail -30 /tmp/weston.out 2>/dev/null || echo "(none)"
echo "=== END weston.out ==="
echo PROBE_DONE
