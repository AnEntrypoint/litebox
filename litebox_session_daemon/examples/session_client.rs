...
use litebox_session_daemon::protocol::{Request, Response, PIPE_NAME};
...
fn main() {
    ...
    let Response::CreateSession { session_id: echo_id } = call(
        handle,
    ...
    // section -- `cat` is used here as a stand-in interactive program: still a real pty-attached
    // guest process reading stdin and echoing to stdout live, exercising the exact same
    // SendInput/GetScreen code path a shell session would).
