...
//! `docs/session-daemon-design.md`. `session start`/`send`/`screen`/`history`/`list`/`kill` on
//! `litebox_runner_linux_on_windows_userland` are thin clients over this same wire protocol (see
//! `client.rs`) -- `examples/session_client.rs` is this crate's own direct test client, exercising
//! identical `pipe_io`/`protocol` plumbing.
...
// `keys` is the only item in this crate usable off Windows (pure string parsing, no Windows
// dependency) -- kept reachable on every target so CI's Linux-hosted `--all-targets
// --all-features` build can exercise it even though the rest of this crate is Windows-only.
pub mod keys;
...
mod daemon {
    ...
    fn create_and_accept_one_instance() -> std::io::Result<PipeHandle> {
        let name = utf16_nul(crate::protocol::PIPE_NAME);
        // SAFETY: `name` is a valid, NUL-terminated UTF-16 string alive for the duration of this
        // call. All other arguments are plain values with no aliasing/lifetime requirements.
        let handle = unsafe {
            CreateNamedPipeW(
                name.as_ptr(),
        ...
        // `ConnectNamedPipe` below can be interrupted/observed the same way any other overlapped
        // I/O on this handle could be in a future revision; this phase does not use overlapped
        // `ReadFile`/`WriteFile` (see `pipe_io.rs`, which passes a null `OVERLAPPED` pointer --
        // valid for a synchronous, blocking call even on a handle opened with
        // `FILE_FLAG_OVERLAPPED`, since the flag only changes I/O *semantics* the caller opts into
        // via a non-null `OVERLAPPED`).
    ...
    fn handle_connection(handle: PipeHandle, registry: Arc<Registry>, runner_exe: Arc<String>) {
        loop {
            let req: Request = match crate::pipe_io::read_message(handle.0) {
                Ok(Some(r)) => r,
                // Client disconnected cleanly between messages, or a real I/O error -- either
                // way, this connection is done.
                Ok(None) | Err(_) => return,
            };
            let resp = dispatch(&registry, &runner_exe, req);
            if crate::pipe_io::write_message(handle.0, &resp).is_err() {
                return;
            }
        }
    }
    ...
    fn dispatch(registry: &Registry, runner_exe: &str, req: Request) -> Response {
        match req {
            ...
            Request::GetHistory { session_id, since } => {
                match registry.history(&session_id, since) {
    ...
    /// Runs the daemon's accept loop forever: one thread per connected client, matching the
    /// design doc's "one connection per CLI invocation" client shape (a short-lived client
    /// connects, sends one request, reads one response, disconnects -- though this loop also
    /// happily serves a longer-lived test client that sends several requests over one connection,
    /// per `read_message`'s natural per-message loop in `handle_connection`).
    ///
    /// `runner_exe` is the path to `litebox_runner_linux_on_windows_userland.exe` used to spawn
    /// every session's guest process.
