...
**Concrete next step for whoever continues**: find weston's actual fallback/default-background-view creation code (NOT `background_committed` -- confirmed a different codepath given the PID mismatch) in the real weston source already fetched this investigation (`weston_compositor.c`, `weston_shell.c`, search terms tried without a hit this sub-session: `add_default_fill_color`, `create_black_surface`, `fill_color` -- try `weston_output_capture`, `weston_output_set_scale`, or simply grep for where `shell->background_layer`/`sh_output->background_view` gets a NON-NULL value assigned OUTSIDE of `background_committed` specifically, since that's the exact assignment this session traced but couldn't find an alternate call site for in the time remaining). Once found, cross-reference why the CLIENT's own `background_committed` path is never reached -- with weston's compositor now stable for 300+ seconds, re-run the SAME live wire-protocol-decoding technique this investigation used earlier (manually decoding `sys_sendmsg`/`sys_recvmsg` raw bytes between weston and desktop-shell, `fd=29` on weston's side per this same sub-session's earlier finding) specifically searching for whether desktop-shell ever sends a `wl_surface.attach`/`wl_surface.commit`/`wl_shm_pool.create_buffer` request at all, now that there's ample stable runtime to observe it happening (or definitively NOT happening) rather than the process dying before it had the chance.
