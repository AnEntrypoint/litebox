...
**Concrete next step, not yet attempted**: with BOTH deadlocks now fixed and weston's compositor loop confirmed genuinely alive, capture a FRESH, longer (60s+) `LITEBOX_LOG=debug` trace and specifically look for (a) whether weston ever sends ANY `wl_callback.done` (frame completion) event to desktop-shell's `fd=26` connection at all (search `sys_sendmsg tid=1000 fd=25` payloads for the frame-callback opcode, or add a targeted decode of the Wayland wire protocol's `wl_callback::done` event specifically, since the existing wire-trace investigation from earlier in this session already has the decoding groundwork), and (b) whether weston's OWN internal repaint scheduling ever proceeds far enough to actually call its output's repaint/damage-tracking logic (not just flip an unchanged buffer) -- i.e. distinguish "weston is flipping the same never-painted buffer 45 times" from "weston is genuinely repainting new content but the content is still black for a different reason" by comparing two consecutive framebuffer's pixel contents directly (the dumb-buffer's raw bytes are host-mappable via the existing DRM dumb-buffer code in `litebox_shim_linux/src/syscalls/drm.rs`).
