...
**However, adding `xfdesktop` back into the mix (even with a full 60s weston warmup before launching it, matching the newly-discovered timing requirement) REGRESSED this** -- desktop-shell's own `launching '/usr/libexec/weston-desktop-shell'` line never even appeared in that run's log at all (a genuinely different, earlier failure than anything seen before), `xfdesktop` itself still failed with `cannot open display: :0` (its `--xwayland` connection), and the window went back to solid black/the benign white-black host-window-sizing split, with one secondary-process (`pid=6`, almost certainly `Xwayland` itself, consistent across two separate runs now) `SIGSEGV` at t=4.1s. **This means the bare-weston breakthrough does NOT yet generalize to the full XFCE stack** -- adding `xfdesktop`/XWayland changes the process tree's own timing and/or introduces the `Xwayland`-specific crash, preventing even desktop-shell's own previously-reliable panel from launching. Given the extensive time already invested this pass and this being a genuinely different, not-yet-diagnosed failure mode (worth its own dedicated investigation, likely starting with why `Xwayland` itself crashes at exactly t=4.1s every time it's included), stopping here for this pass with the bare-weston panel breakthrough as the session's real, verified, reproducible (given adequate wait time) achievement, and the full-XFCE-stack integration as the clearly-scoped next step for whoever continues.
