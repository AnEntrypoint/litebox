...
**Concrete next step, not yet attempted**: get a tar combination with COMPLETE, valid xkb rule data (the `alpine-pinned2.tar` base has real `/usr/share/X11/xkb/rules/evdev.xml` etc., confirmed present via direct `tar tf`, but the running overlay combination still failed to parse a keymap from it -- likely a missing `/etc/xkb` symlink AND missing default `XKB_DEFAULT_RULES`/`XKB_DEFAULT_LAYOUT` env vars together, not just `XKB_CONFIG_ROOT` alone) and re-run the full XFCE repro to see whether a fully-working keyboard context is what unblocks desktop-shell's own `wl_surface` creation, before returning to the original fork_verify/wire-trace investigation track if it isn't.
