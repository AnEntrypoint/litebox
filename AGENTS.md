...
**Concrete next step**: instrument `ReadySet::push`/`EpollEntry::is_ready`'s exact state transitions across weston's repeated `timerfd_settime` re-arm calls (already logged, see `sys_timerfd_settime: entry tid=22 fd=9` -- 5+ re-arms happen in `timerfd_fix_run1.log`, the LAST at t=27.828763200s with `value=26.863053s`, which per this fix's mechanism SHOULD have triggered a repoll-driven ready-push at its computed deadline but evidently didn't) to find exactly where the state machine diverges from the first several (working) cycles.
