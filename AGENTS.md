...
**Concrete next step for whoever continues**: attach a REAL debugger (gdb via `cdb`-equivalent Linux tooling, or add a one-line `fprintf(stderr, ...)` fork of `weston-desktop-shell.c` compiled locally and swapped into the tar -- the one legitimate exception to "don't patch the guest," since it's a TEMPORARY diagnostic build, not a permanent OS modification) directly inside `create_output`/`output_init`/`main()`'s catch-up loop to log `desktop->shell`'s actual pointer value and whether `output_init` is called at all, at the exact moment `create_output` runs for the `Virtual-1` output. This is the only way to distinguish "the function never gets called" from "it's called but exits/fails silently inside `background_create`/`weston_desktop_shell_set_background`" without further blind source-reading -- direct instrumentation of the ACTUAL failing binary, temporarily, is the correct next tool now that every host-side avenue is exhausted.
