...
This means `main()`'s own catch-up loop (`wl_list_for_each(output, ...) if (!output->background) output_init(...)`, `clients/desktop-shell.c` ~line 1590) is THE piece that must be failing -- either it never runs (a control-flow bug reaching `main()`'s own code, or `display_run()` being entered before this loop for some reason), or it runs but doesn't find the output it should (e.g. a `desktop.outputs` list-corruption or ordering issue). **Concrete, sharply narrowed next step for whoever continues**: this is now down to ONE specific, small function in real, already-fetched weston client source (`main()`'s catch-up loop, `clients/desktop-shell.c` ~line 1588-1592) -- either instrument it directly (once the separate `cc1` compile blocker is resolved) with an `fprintf` right before/after the loop and its `output_init()` call, or reason about whether `desktop.shell` is genuinely non-NULL by the time this loop runs (it should be, since `display_set_global_handler` fully replays and processes every global including `weston_desktop_shell`'s bind before returning, per this pass's own read of `window.c`) -- if `desktop.shell` IS set correctly there, the remaining question narrows further to whether `wl_list_for_each(output, &desktop.outputs, link)` genuinely iterates the output this session's trace shows was created, which would point at a `wl_list`/`desktop.outputs` state bug specific to litebox's guest environment rather than anything in weston's own logic.
