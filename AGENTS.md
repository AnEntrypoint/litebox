...
**Concrete next step for whoever continues**: investigate WHY weston's own internal event loop / desktop-shell module (`desktop-shell/shell.c`, real source already fetched this investigation via `gh api`) stalls after the initial client roundtrip and never sends the expected follow-up event(s) to advance the handshake. This is now a weston-compositor-logic question, not a litebox-syscall question -- check weston's OWN internal state machine for `wl_shell`/`desktop-shell` global binding: does it wait on some other precondition (output layout finalized, `xdg_output` handshake with a DIFFERENT client completing first, a compositor-internal idle/repaint scheduling gate) before it will send desktop-shell its next event? Cross-reference weston's own log output (`sys_write tid=22 fd=2`) around and after t=14.14s for any diagnostic/warning it prints about desktop-shell or background-surface setup. The earlier `[13:39:21.446] Warning: computed repaint delay for output [Virtual-1] is abnormal: -12588 msec` (t=13.471298200s, well before the freeze) may be a related clue -- a negative/abnormal repaint delay could indicate weston's internal repaint scheduler is in a bad state that prevents it from ever reaching the point where it'd send desktop-shell further events.
